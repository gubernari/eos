...
namespace eos
{
    ...
    struct Implementation<BToKstarCharmonium>
    {
        ...
        Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
            ...
            correlator(o, "correlator", { "GvDV2019" }, "GvDV2019"),
            nonlocal_correlator(NonlocalCorrelator<nc::PToV>::make(correlator.value(), p, o)),
        ...
        {
            if (! nonlocal_correlator.get())
                throw InternalError("Cannot construct the nonlocal correlator");

            if ("J/psi" == psi.value())
            {
                residue_H_long = std::bind(&NonlocalCorrelator<nc::PToV>::H_long_residue_jpsi, nonlocal_correlator);
                residue_H_perp = std::bind(&NonlocalCorrelator<nc::PToV>::H_perp_residue_jpsi, nonlocal_correlator);
                residue_H_para = std::bind(&NonlocalCorrelator<nc::PToV>::H_para_residue_jpsi, nonlocal_correlator);
            }
            else
            {
                residue_H_long = std::bind(&NonlocalCorrelator<nc::PToV>::H_long_residue_psi2s, nonlocal_correlator);
                residue_H_perp = std::bind(&NonlocalCorrelator<nc::PToV>::H_perp_residue_psi2s, nonlocal_correlator);
                residue_H_para = std::bind(&NonlocalCorrelator<nc::PToV>::H_para_residue_psi2s, nonlocal_correlator);
            }

            u.uses(*model);
            u.uses(*nonlocal_correlator);
            u.uses(*form_factors);
        }
